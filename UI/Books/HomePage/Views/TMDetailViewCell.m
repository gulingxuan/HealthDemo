   ...
   self = [super initWithStyle:style reuseIdentifier:reuseIdentifier];
    if (self) {
        ...
        [self.timePointView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.centerX.equalTo(weakSelf.contentView);
            make.centerY.mas_equalTo(weakSelf.containerCategoryImageAndLabelView.mas_top).offset(-15).priorityLow();
        }];
        ...
        [self.timeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.right.mas_equalTo(weakSelf.timePointView.mas_left).offset(-10);
        }];
...
- (void)updateTimePointAndContainerViewConstraints {
    ...
    [self.containerCategoryImageAndLabelView mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(SCREEN_SIZE.width, kDetailViewContainerViewHeight));
        make.centerX.equalTo(weakSelf.contentView);
        make.top.mas_equalTo(weakSelf.timePointView.mas_bottom).offset(15);
    }];
}
/** 修改备注照片的布局 */
- (void)updatePictureImageViewConstraintsWithHave:(BOOL)have {
    ...
    if (have) {
        [self.pictureImageView mas_remakeConstraints:^(MASConstraintMaker *make) {
            ...
            make.centerX.equalTo(weakSelf.contentView);
            make.top.mas_equalTo(weakSelf.containerCategoryImageAndLabelView.mas_bottom);
        }];
    } else {
        [self.pictureImageView mas_remakeConstraints:^(MASConstraintMaker *make) {
...
- (void)updateRemarkLabelConstraintsWithBottom:(BOOL)bottom {
    ...
    if (bottom) {//底部
        [self.remarkLabel mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(weakSelf.pictureImageView.mas_bottom);
            make.size.mas_equalTo(CGSizeMake(SCREEN_SIZE.width - 50, 65));
            make.centerX.equalTo(weakSelf.contentView);
        }];
        [self.containerRemarkLabelView mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.width.equalTo(@0);
        }];
    } else {//中部
        [self.containerRemarkLabelView mas_remakeConstraints:^(MASConstraintMaker *make) {
