...
@property (nonatomic, strong) UIButton *backFatXuan;
@end
...
-(UIButton *)backFatXuan{
    if (!_backFatXuan) {
        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = CGRectMake(20, SCREEN_SIZE.height-60, 40, 40);
        [btn setBackgroundImage:[UIImage imageNamed:@"backFatXuan"] forState:UIControlStateNormal];
        [btn addTarget:self action:@selector(onClickCamerBtn:) forControlEvents:UIControlEventTouchUpInside];
        _backFatXuan = btn;
    }
    return _backFatXuan;

}
-(void)onClickCamerBtn:(UIButton *)sender{
    UIWindow *widonw = [[UIApplication sharedApplication]keyWindow];
    TabBarViewController *tvc = [[TabBarViewController alloc]init];
    widonw.rootViewController = tvc;
}
#pragma mark - life
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    [self.view addSubview:self.backFatXuan];
